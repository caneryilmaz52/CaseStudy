package com.scorpapp.casestudy.data
...
class DataSource {
    ...
    private object Constants {
        ...
        val fetchCountRange: ClosedRange<Int> = 20..30 // lower bound must be > 0, upper bound must be > lower bound
    ...
    private fun processRequest(next: String?): ProcessResult {
        ...
        if (isError) {
            ...
            if (next != null && (nextIntValue == null || nextIntValue < 0)) {
                ...
                var fetchedPeople: ArrayList<Person> = ArrayList(people.subList(beginIndex, endIndex)) // begin ile end ayni olunca bos donuyor mu? - Cevap evet :)
